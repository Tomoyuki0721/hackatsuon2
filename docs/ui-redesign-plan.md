...
1. 共通レイアウト(`AppShell`, `AppHeader`, `Sidebar`)
2. `ProjectHeader` + `DisplayModeTabs`
3. 予算モード通常表示(`KpiCard`, `SectionCard`, `NewProjectCard`, `FinanceDonutChart`を新規作成して使用)
4. → ここでGitHub Pagesにデプロイし、表示を確認してもらってからレイアウト崩れ・情報密度を調整
5. 予算詳細 → 決算通常/詳細 → 一般質問通常/詳細 → 市民通常/詳細 の順で残りを実装
