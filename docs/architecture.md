...
既存の `index.html` (ワークショップ初心者向けスターター)は、Next.js化に伴い役割が競合する。
`output: export` のビルド成果物は `out/index.html` として生成されるため、**リポジトリ直下の
手書き `index.html` は削除または `legacy/` 等に退避する必要がある**(GitHub Pagesはリポジトリ
直下の `index.html` を優先して配信するため、残したままだとNext.js側のトップページが表示されない)。
この対応はPhase 6でNext.jsのトップページが動く状態になった段階で行う(今は既存ファイルを壊さない)。
